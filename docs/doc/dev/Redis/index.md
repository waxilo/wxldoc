...
每个设置了过期时间的redisKey绑定一个定时器，到期立即删除。
...
每隔一段时间，随机抽取一批键，检查其中过期键进行删除；如果删除比例超过25%，再次抽取。
...
**缺点**：如果键一直不使用，内存一直无法释放。



**Redis实际使用**：**惰性删除 + 定期删除** 结合



## Redis淘汰策略

当内存达到 `maxmemory`限制时触发：
...
| 策略               | 说明                       | 适用场景         |
| ------------------ | -------------------------- | ---------------- |
| **allkeys-lru**    | 所有键中淘汰最近最少使用的 | 热点数据明显     |
| **allkeys-lfu**    | 所有键中淘汰访问频率最低的 | 需要分析访问频率 |
| **allkeys-random** | 随机淘汰任意键             | 数据访问均匀     |
...
| 策略                | 说明                           | 适用场景          |
| ------------------- | ------------------------------ | ----------------- |
| **volatile-lru**    | 从设置了过期时间的键中淘汰LRU  | 混合存储场景      |
| **volatile-lfu**    | 从设置了过期时间的键中淘汰LFU  | 混合存储+频率分析 |
| **volatile-random** | 从设置了过期时间的键中随机淘汰 | 混合存储          |
| **volatile-ttl**    | 淘汰剩余存活时间最短的键       | 优先保留长期数据  |
...
一个主节点处理写操作，多个从节点处理读操作。

**优点**：部署简单。

**缺点**：当主节点宕机，存在单点故障，需手动配置主节点。
